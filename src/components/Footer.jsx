...
const Footer = () => {
  ...
  // Нормализуем путь: убираем завершающий слэш (кроме "/")
  ...
  // Находим "активную" страницу по самому длинному совпадению префикса
  // (чтобы /blog/post/123 корректно считался как "Блог")
  const { activePageName, filteredMainLinks } = useMemo(() => {
    const allKnownLinks = [...mainLinks, ...services, ...company];

    const match = allKnownLinks
      .map((l) => ({ ...l, path: normalizePath(l.path) }))
      .filter((l) => l.path === '/' ? pathname === '/' : pathname === l.path || pathname.startsWith(l.path + '/'))
      .sort((a, b) => b.path.length - a.path.length)[0];

    const activeName = match?.name || 'Страница';

    // Убираем текущую страницу из списка главных ссылок (точное совпадение / и так далее)
    const filtered = mainLinks
      .map((l) => ({ ...l, path: normalizePath(l.path) }))
      .filter((l) => !(l.path === '/' ? pathname === '/' : pathname === l.path));

    return { activePageName: activeName, filteredMainLinks: filtered };
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p className="text-gray-300">{getCompanyAddress()}</p>
      ...
      <div className="border-t border-gray-800">
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="flex justify-center mb-4">
            <div className="text-xs text-gray-400 bg-gray-800/60 border border-gray-700 px-3 py-1 rounded-full">
              Сейчас: <span className="text-white">{activePageName}</span>
            </div>
          </div>

          {/* Главные ссылки (без текущей страницы) */}
          <div className="flex flex-wrap justify-center gap-3 mb-6 text-sm">
            {filteredMainLinks.map((link, index) => (
              <React.Fragment key={link.path}>
                ...
                {index < filteredMainLinks.length - 1 && (
