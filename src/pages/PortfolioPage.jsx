...
=======
  Loader
>>>>>>> a017c62b33947a2946db1e395a2df2d4880cc344
} from 'lucide-react';
...
=======
  ...
  useEffect(() => {
    const fetchPortfolioData = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://127.0.0.1:5000/api/portfolio/'); // TODO: обязательно заменить на хост в будущем
        console.log('Загрузка данных портфолио с API:', response);
        console.log('данные взяты с локального хоста');
        if (!response.ok) {
        ...
        setError('Не удалось загрузить данные портфолио. Попробуйте позже.');
        setPortfolioData([]);
        setPagination(null);
      } finally {
        setLoading(false);
...
>>>>>>> a017c62b33947a2946db1e395a2df2d4880cc344
  ...
  //статичные данные на случай очистки бд
  // const portfolioData = [
  //   {
  //     id: 1,
  //     title: 'День рождения принцессы Амелии (5 лет)',
                      ...
                      <div className="p-6 flex flex-col flex-grow">
                        ...
                        <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                          <div className="flex items-center gap-1">
                            <Calendar size={16} />
                            <span className="truncate">{new Date(project.date).toLocaleDateString('ru-RU')}</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <Users size={16} />
                    ...
                    <>
                      ...
                      <div className="flex-grow min-w-0">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          {project.featured && (
                            <span className="bg-yellow-500 text-white text-xs font-bold px-2 py-1 rounded-full flex-shrink-0">
                              ТОП
                            </span>
                          )}
                        </div>
